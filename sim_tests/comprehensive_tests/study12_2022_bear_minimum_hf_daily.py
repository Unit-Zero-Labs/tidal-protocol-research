#!/usr/bin/env python3
"""
Study 12: 2022 Bear Market - Minimum Viable Health Factor (Daily Rebalancing)

Optimization study to find the absolute minimum health factor that could survive
the 2022 bear market on Aave with daily rebalancing.
...
class MinimumHFOptimizer:
    ...
    def find_minimum_hf(self) -> Tuple[float, Dict[str, Any]]:
        ...
        print(f"Strategy: Daily rebalancing with historical Aave rates")
        print("=" * 80)
        print()
    ...
    def _test_health_factor(self, test_hf: float) -> Tuple[bool, int]:
        ...
        # DAILY REBALANCING
        config.leverage_frequency_minutes = 1440  # 1 day = 1,440 minutes
        
        # Disable unnecessary features for speed
        config.use_advanced_moet = False
        config.enable_weekly_yield_harvest = False  # Daily rebalancing, no weekly override
    ...
    def _save_test_iteration_data(self, test_hf: float, survived: bool, liquidation_minute: int, results: Dict[str, Any]):
        ...
        try:
            ...
            output_dir = Path(__file__).parent.parent.parent / "tidal_protocol_sim" / "results" / "Study_12_2022_Bear_Minimum_HF_Daily_Test_Data"
...
def run_comparison_with_optimal_hf(optimal_aave_hf: float, optimization_summary: Dict[str, Any]):
    ...
    Both with daily rebalancing during 2022 bear market.
    """
    print("\n" + "=" * 80)
    ...
    print(f"  - Strategy: Daily rebalancing")
    ...
    # Study 12 parameters
    config.test_name = f"Study_12_2022_Bear_Minimum_HF_Daily_Aave_{optimal_aave_hf:.2f}_vs_HT_1.20"
    config.simulation_duration_days = 365
    ...
    # No weekly yield harvesting for daily study
...
def main():
    print("\n" + "🎯" * 40)
    print("STUDY 12: 2022 Bear Market - Minimum Viable Health Factor (Daily)")
    print("🎯" * 40)
    print()
    print("Objective: Find the absolute minimum health factor that could have")
    print("           survived the 2022 bear market on Aave with daily rebalancing.")
    print()
    ...
    print("\n" + "=" * 80)
    print("STUDY 12 COMPLETE - KEY FINDINGS")
    print("=" * 80)
    print()
    ...
    print(f"   tidal_protocol_sim/results/Study_12_2022_Bear_Minimum_HF_Daily_Aave_{optimal_hf:.2f}_vs_HT_1.20/")
    print()
    print("=" * 80)
